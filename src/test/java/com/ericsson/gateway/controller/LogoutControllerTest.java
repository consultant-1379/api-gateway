...
class LogoutControllerTest extends BasicSpringBootTest {
  ...
  private WebFilter securityCtxFilter =
  ...
  private WebFilter authorizedClientFilter =
      (serverWebExchange, webFilterChain) ->
          serverWebExchange
              .getSession()
              .doOnNext(
                  webSession1 ->
                      createAuthorizedClient(createAuthenticatedUserToken(), serverWebExchange))
              .then(webFilterChain.filter(serverWebExchange));
  ...
  void shouldReturnErrorResponseWhenLogoutError() {
    ...
    client.post().uri("/auth/v1/logout").exchange().expectStatus().is5xxServerError();
  }
